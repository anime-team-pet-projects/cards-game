...
const { startTimer, stopTimer, subscribe } = useTimer()
...
audioLoader.load('./card_drop.mp3', function(buffer) {
  cardDrop.setBuffer(buffer);
  cardDrop.setVolume(2);
});

audioLoader.load('./card_flip.mp3', function(buffer) {
...
window.addEventListener('click', function(e) {
  ...
  if(hoveredCard && playNext && !finished) {
    ...
    socket.on('opponentMove', (data) => {
      ...
      const randomNumber = Math.floor(Math.random() * opponentCards.length);
      const oppCName = opponentCards[0].name;
      ...
      tl2.to(opponentCards[randomNumber].rotation, {
        ...
        .to(opponentCards[randomNumber].position, {
          ...
          onComplete: function () {
            playNext = true;
            // Update the game state based on the cards played
            if (hoveredCard.name.includes('emperor') && oppCName.includes('slave'))
              resetAndUpdate('opponent', oScore);
            if (hoveredCard.name.includes('emperor') && oppCName.includes('citizen'))
              resetAndUpdate('player', pScore);
            if (hoveredCard.name.includes('citizen') && oppCName.includes('slave'))
              resetAndUpdate('player', pScore);
            if (hoveredCard.name.includes('slave') && oppCName.includes('emperor'))
              resetAndUpdate('player', pScore);
            if (hoveredCard.name.includes('slave') && oppCName.includes('citizen'))
              resetAndUpdate('opponent', oScore);
            if (hoveredCard.name.includes('citizen') && oppCName.includes('emperor'))
              resetAndUpdate('opponent', oScore);
          }
        }, 0);
